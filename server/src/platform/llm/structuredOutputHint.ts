...
function resolveArrayExampleLength(schema: AnySchema): number {
  const checks = (schema._def as { checks?: Array<{ value?: unknown; kind?: string }> })?.checks ?? [];
...
function buildObjectExample(
  ...
  const rawShape = safeShape(schema._def);
  if (!rawShape) return {};
  return Object.fromEntries(
...
function buildRecordExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): Record<string, unknown> {
  const valueSchema = (schema as AnySchema & {
    _def?: { valueType?: AnySchema; valueSchema?: AnySchema; };
...
function buildMapExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): unknown[] {
  const valueSchema = (schema as AnySchema & { _def?: { valueType?: AnySchema; value?: AnySchema; }; })._def?.valueType
    ?? (schema as AnySchema & { _def?: { valueType?: AnySchema; value?: AnySchema; }; })._def?.value;
...
function buildSetExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): unknown[] {
  const valueSchema = (schema as AnySchema & { _def?: { valueType?: AnySchema; type?: AnySchema; }; })._def?.valueType
    ?? (schema as AnySchema & { _def?: { valueType?: AnySchema; type?: AnySchema; }; })._def?.type;
...
function buildTupleExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): unknown[] {
  const tupleItems = (schema as AnySchema & { items?: AnySchema[]; _def?: { items?: AnySchema[]; }; }).items
    ?? (schema as AnySchema & { items?: AnySchema[]; _def?: { items?: AnySchema[]; }; })._def?.items ?? [];
...
function buildUnionExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): unknown {
  const options = (schema as AnySchema & { options?: AnySchema[]; _def?: { options?: AnySchema[]; }; }).options
    ?? (schema as AnySchema & { options?: AnySchema[]; _def?: { options?: AnySchema[]; }; })._def?.options ?? [];
...
function buildDiscriminatedUnionExample(schema: AnySchema, depth: number, seen: Set<AnySchema>): unknown {
  const options = (schema as AnySchema & {
    ...
    _def?: { options?: Map<string, AnySchema> | Set<AnySchema> | AnySchema[]; };
...
function buildExampleFromSchema(
  ...
  try {
    ...
    if (current instanceof z.ZodString) { return "示例文本"; }
    if (current instanceof z.ZodNumber) { return 1; }
    if (current instanceof z.ZodBigInt) { return 1; }
    if (current instanceof z.ZodBoolean) { return true; }
    if (current instanceof z.ZodDate) { return "2026-01-01T00:00:00.000Z"; }
    if (current instanceof z.ZodNull) { return null; }
    if (current instanceof z.ZodUndefined || current instanceof z.ZodVoid) { return null; }
    if (current instanceof z.ZodAny || current instanceof z.ZodUnknown || current instanceof z.ZodNever) { return "示例值"; }
    if (current instanceof z.ZodNaN) { return 0; }
...
function resolveStructuredOutputExample<I, O, R>(
  ...
  if (typeof customExample === "function") { return customExample(promptInput, context); }
  if (customExample !== undefined) { return customExample; }
  if (!asset.outputSchema) { return {}; }
...
function resolveStructuredOutputNote<I, O, R>(
  ...
  if (typeof customNote === "function") { return customNote(promptInput, context) ?? ""; }
  return customNote ?? "";
}

function buildStructuredOutputHintText(example: unknown, customNote: string): string {
  const noteLine = customNote.trim() ? `- ${customNote.trim()}` : "";
