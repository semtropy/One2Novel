...
**跨章多级弧线：** 短期悬念（3-8章）→埋设→施压→部分兑现；中期悬念（15-30章，回环级）→多轮施压→阶段性揭示；长期悬念（50-150章，卷级核心谜团）→缓慢释放线索→卷末重大兑现；超长期悬念（200-500章，全书终极谜团）→贯穿始终的暗线
...
## 故事结构模板
**三幕式（长篇嵌套版）：** 全书级：建立(~10%)→对抗(~60%)→解决(~30%)。每卷内嵌套：建立(15-20%)→对抗(55-65%)→解决(15-25%)。每回环内再嵌套微型三幕。长篇网文的整体比例偏向更长中段（持续升级）和更短建立期。
