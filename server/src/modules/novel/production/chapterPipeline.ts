 ...
 *
 * Used by both the SSE manual-write path (chapterWriter.ts) and the
 * batch auto-write path (directorService.ts).
 */

import { getPrisma } from "../../../platform/db/client";
import { buildCharacterProhibitions, assembleRepairContext } from "./contextAssembler";
import { assembleChapterContext } from "./contextAssembler";
import { runQualityGate, totalQualityScore } from "./qualityGate";
import { persistQualityScores } from "./qualityPersist";
import { runPostWriteHooks } from "./postWriteHooks";
import { finalizeChapter } from "./finalization";
import { diagnoseWorkspace } from "./revisionService";
import { formatIssuesForRepair, patchRepair, heavyRepair } from "./repairService";
...
export async function processChapter(
  ...
  const [novelGenre, chapterExpectation, charProhibitions, prevChapter] = await Promise.all([
    ...
    buildCharacterProhibitions(novelId).catch(() => undefined),
