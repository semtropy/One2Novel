...
/**
 * Get a commit by novel ID and chapter order.
 */
export async function getCommitByOrder(novelId: string, chapterOrder: number): Promise<{
  id: string;
  chapterId: string;
  status: string;
  projectionStatus: string;
  ...
  const prisma = getPrisma();
  return prisma.chapterCommit.findFirst({
    where: { novelId, chapterOrder, status: { not: 'rejected' } },
    orderBy: { createdAt: 'desc' },
    select: { id: true, chapterId: true, status: true, projectionStatus: true },
  });
}

/**
 * Get all unfinished commits for a novel (for replay).
 */
export async function getUnfinishedCommits(novelId: string): Promise<Array<{
  id: string;
  chapterId: string;
  chapterOrder: number;
  projectionStatus: string;
  ...
  const prisma = getPrisma();
  return prisma.chapterCommit.findMany({
    where: {
      novelId,
      status: 'accepted',
      projectionStatus: { in: ['not_started', 'in_progress', 'partially_completed', 'failed'] },
    },
    select: { id: true, chapterId: true, chapterOrder: true, projectionStatus: true },
    orderBy: { chapterOrder: 'asc' },
  });
}

/**
 * Create a retroactive commit for chapters written before Phase 3.
 * Used for backward compatibility — assumes projection was already done.
 */
export async function createRetroactiveCommit(
  chapterId: string,
  novelId: string,
  chapterOrder: number,
  qualityScore?: number,
  ...
  const commit = await prisma.chapterCommit.create({
    data: {
      ...
      status: 'accepted',
      acceptedAt: new Date(),
      projectionStatus: 'completed', // Assume already projected
    },
  });
  return commit.id;
}
...
/**
 * Check if a commit exists for a chapter.
 */
export async function hasCommit(chapterId: string): Promise<boolean> {
  const prisma = getPrisma();
  const count = await prisma.chapterCommit.count({
    where: { chapterId },
  });
  return count > 0;
}
