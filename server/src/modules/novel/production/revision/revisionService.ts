...
/**
 * Cascade revision — revise a chapter and mark subsequent chapters for review.
 * Instead of auto-regenerating all subsequent chapters (expensive and risky),
 * this approach lets the user revise the current chapter and flags downstream
 * chapters as potentially affected, so they can be manually reviewed.
 */
export async function cascadeRevision(
  novelId: string,
  chapterId: string,
  instruction: string,
  scope: "single" | "cascade" = "single",
): Promise<{ revisedChapterId: string; affectedChapters: number; editHistoryId: string }> {
  const prisma = getPrisma();
  const chapter = await prisma.chapter.findUnique({
    where: { id: chapterId },
    select: { id: true, order: true, content: true, scenePlan: true },
  });
  if (!chapter?.content) throw new Error("Chapter has no content");

  // Save snapshot before modification
  const history = await prisma.chapterEditHistory.create({
    data: {
      chapterId: chapter.id,
      content: chapter.content,
      sceneState: chapter.scenePlan,
    },
  });
  ...
  if (scope === "cascade") {
    // Mark subsequent chapters as "needs_repair" for manual review
    const subsequent = await prisma.chapter.findMany({
      where: { novelId, order: { gt: chapter.order }, chapterStatus: "completed" },
      select: { id: true },
    });

    if (subsequent.length > 0) {
      await prisma.chapter.updateMany({
        where: { id: { in: subsequent.map((s) => s.id) } },
        data: {
          chapterStatus: "needs_repair",
          expectation: `[级联修订影响] 第${chapter.order}章已被修订(${instruction.slice(0, 100)})，本章可能需要调整连续性。`,
        },
      });
      affectedChapters = subsequent.length;
    }
  }

  return {
    revisedChapterId: chapter.id,
    affectedChapters,
