...
promptRegistry.register({
  ...
  systemPrompt: [
    ...
    "",
    "【Few-Shot 示例：一条完整技法提取的格式】",
    `{"overallStyleDescription":"冷峻克制的第三人称叙述，以客观描写推进剧情，避免大段心理独白。对话简洁有力，战斗场景用短句营造紧张感。","narrativeAssets":[{"category":"视角切换","observation":"对齐标书中，主角逃亡时切换到追捕队长的视角，通过队长的报告间接展示主角能力的诡异之处，不需要主角视角解释。","rule":"在展示主角底牌时，切换到对手或旁观者视角，通过他们的观察和反应来侧面烘托，避免主角自言自语解释能力。","confidence":0.9}],"languageAssets":[{"category":"句式长短","observation":"战斗场景使用连续短句（每句5-15字），日常场景使用中长句（15-30字），形成节奏对比。","rule":"动作场景中每句不超过15字，不要用连接词串联动作。日常场景允许20-30字的中长句用于环境描写和角色对话。","confidence":0.85}],"characterAssets":[{"category":"角色反应模式","observation":"主角在被背叛时没有心理描写——只写他'沉默了很久'然后'把酒杯放在桌上，没喝完'。情感通过动作间接表达。","rule":"角色情感变化不直接陈述（'他很愤怒'），改用具体动作（'握紧的指节发白'）或行为变化（'没喝完的酒'）来间接表现。","confidence":0.9}]}`,
  ].join("\n"),
});
...
promptRegistry.register({
  ...
  systemPrompt: [
    ...
    "- 权谋：政治博弈/势力对抗/阴谋策划/情报战争/离间计",
    "- 战争：大规模军事冲突/攻城战/种族战争/领地争夺",
    "- 建设：领地发展/宗门经营/城市建设/产业扩张/军队招募",
    "- 探索：未知领域开拓/秘境探险/位面穿越/遗迹发掘",
    "- 传承：获得遗产/继承教义/师徒授业/血脉觉醒/世界级奥秘传承",
    "",
    "## 分析粒度",
