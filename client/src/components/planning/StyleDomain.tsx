/**
 * StyleDomain — manage style profiles: create, extract features, edit rules.
 *
 * Props: novelId — reserved for future binding context (not currently used
 * in the profile list view, which shows all global profiles).
 */
import { useState } from "react";
import {
  Sparkles, Trash2, Plus, FileText, RefreshCw, Pencil, X,
...
function parseRules(profile: any, field: string): string[] {
  try {
    const arr = JSON.parse((profile[field] as string) ?? "[]");
...
export function StyleDomain({ novelId: _novelId }: { novelId?: string }) {
  ...
  return (
    <div>
      ...
      <button
        ...
        className="flex items-center gap-2 rounded-lg bg-slate-800 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700 mb-6"
      >
        <Plus size={16} />
        新建写法
      </button>
      ...
      {showCreate && (
        <div className="mb-6 rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => {
                createProfile.mutate({ name, sourceText: text || undefined });
                setName("");
                setText("");
                setShowCreate(false);
              }}
      ...
      <div className="space-y-3">
        {!profiles || profiles.length === 0 ? (
          ...
          profiles.map((p) => {
            const features = (() => {
              try {
                return p.extractedFeatures ? JSON.parse(p.extractedFeatures) : null;
              } catch {
                return null;
              }
            })();
            ...
            return (
              <div key={p.id} className="rounded-xl border border-slate-200 bg-white p-4">
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-1.5">
                    ...
                    {editNameId === p.id ? (
                      <input
                        ...
                        onBlur={async () => {
                          if (editNameVal.trim() && editNameVal !== p.name) {
                            await api.patch(`/styles/${p.id}`, { name: editNameVal.trim() });
                            qc.invalidateQueries({ queryKey: ["styles"] });
                          }
                          setEditNameId(null);
                        }}
                  ...
                  <div className="flex items-center gap-1">
                    {p.sourceText && !p.extractedFeatures && (
                      <button
                        onClick={() => extractStyle.mutate(p.id)}
                        disabled={extractStyle.isPending}
                ...
                {features?.overallDescription && (
                  <p className="text-xs text-slate-500 mb-2">{features.overallDescription}</p>
                )}

                {/* Collapsed view: feature tag pills (max 6) */}
                {!isExpanded && features && (
                  <div className="flex flex-wrap gap-1">
                    {[
                      ...(features.narrativeRules ?? []),
                      ...(features.languageRules ?? []),
                      ...(features.characterRules ?? []),
                    ]
                      .slice(0, 6)
                ...
                {/* Expanded view: 5 field sections */}
                {isExpanded && (
                                ...
                                <div key={i} className="flex items-start gap-1 group">
                                  {editKey === `${field}:${i}` ? (
                                    <input
                                      ...
                                      onKeyDown={(e) => {
                                        if (e.key === "Enter")
                                          handleSaveRule(p.id, field, i, editVal);
                                    ...
                                    <>
                                      ...
                                      <button
                                        ...
                                        className="text-slate-300 hover:text-slate-500 opacity-60 hover:opacity-100 shrink-0"
                                      >
                                        <Pencil size={10} />
                                      </button>
                                      <button
                                        onClick={() => handleDeleteRule(p.id, field, i)}
                                        className="text-slate-300 hover:text-red-400 opacity-60 hover:opacity-100 shrink-0"
                                      >
                                        <X size={10} />
                                      </button>
                                    </>
                          ...
                          {addField === field && (
                            <div className="flex gap-1 ml-8 mt-1">
                              <input
                                ...
                                onKeyDown={(e) => {
                                  if (e.key === "Enter") handleAddRule(p.id, field);
                                  if (e.key === "Escape") setAddField(null);
                                }}
                                placeholder="新规则"
                              />
                              <button
