...
import { Sparkles, RefreshCw, Zap, Check, Loader2, CheckCircle } from "lucide-react";
...
const LOOP_PHASE_LABELS: Record<string, { label: string; hint: string; color: string }> = {
  ...
  explore: { label: "深入展开", hint: "副本内部展开，遭遇挑战", color: "bg-indigo-100 text-indigo-700" },
  setback: { label: "受挫考验", hint: "遭遇重大阻碍或失败", color: "bg-red-100 text-red-700" },
  turn: { label: "转折翻盘", hint: "利用资源/信息逆转局势", color: "bg-purple-100 text-purple-700" },
...
export function BlueprintDomain({ novelId, onComplete }: Props) {
  ...
  const handleGenerateAll = async () => {
    ...
    try { await api.post(`/novels/${novelId}/blueprint`); refetch(); }
    catch (e) { setGenError(e instanceof Error ? e.message : "生成失败"); }
    finally { setGenerating(false); }
  };
  ...
  return (
    <div className="space-y-5">
      ...
      {!skeleton && (
        <section className="rounded-xl border border-dashed border-purple-300 bg-purple-50/30 py-8 text-center space-y-3">
          <p className="text-sm text-purple-700 font-medium">{genMode === "full" ? "一键生成全书蓝图" : "先生成回环骨架"}</p>
          <p className="text-xs text-purple-500">{genMode === "full" ? "AI 将一次性生成全书所有回环并展开为章节" : "生成回环骨架后，逐卷展开为章节"}</p>
          <button onClick={genMode === "full" ? handleGenerateAll : handleGenerateSkeleton} disabled={generating || genSuccess}
            className={cn("rounded-lg px-4 py-2 text-xs font-medium text-white transition-colors disabled:opacity-40",
              genSuccess ? "bg-green-600" : "bg-purple-600 hover:bg-purple-700")}>
          ...
          <div className="overflow-x-auto">
            <div className="flex gap-2 min-w-max">
              {skeleton.loops.map((loop: { loopIndex: number; dungeonName: string; estimatedChapters: number }) => {
                ...
                return (
                  <div key={loop.loopIndex} className={cn("rounded-xl border bg-white min-w-[160px] max-w-[200px] flex flex-col", isExpanded ? "border-green-300" : "border-purple-200")}>
                    <div className="p-3 border-b border-slate-100">
                      <div className="flex items-center justify-between mb-1">
                        <span className="rounded bg-purple-100 px-1.5 py-0.5 text-xs font-medium text-purple-600">第{loop.loopIndex}轮</span>
                      ...
                      {isExpanded ? (
                        ...
                        <button onClick={() => handleExpandVolume(loop.loopIndex)} disabled={isExpanding}
                          className={cn(
                            ...
                            expandSuccess && expandedLoops.has(loop.loopIndex) ? "border-green-300 bg-green-50 text-green-600" : "border-purple-200 text-purple-500 hover:bg-purple-50",
                          )}>
