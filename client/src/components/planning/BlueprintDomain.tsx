...
export function BlueprintDomain({ novelId, onComplete }: Props) {
  ...
  const volumes = (novel?.volumes ?? []) as Array<{
    ...
    chapterPlans: Array<{
      ...
      loopPhase?: string | null; chapterType?: string | null; contentBeat?: string | null;
              ...
              <div className="p-2">
                {vol.chapterPlans.slice(0, 5).map(ch => (
                  <div key={ch.id} className="flex items-center gap-2 px-2 py-1 text-xs">
                    ...
                    {ch.contentBeat && <span className="rounded px-1 py-0 text-[10px] bg-slate-800 text-white shrink-0">{ch.contentBeat}</span>}
