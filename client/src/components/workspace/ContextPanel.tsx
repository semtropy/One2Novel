      ...
      <div className="flex flex-col gap-1">
        {PANELS.map(panel => (
          <button key={panel.key} onClick={() => setActive(panel.key)} disabled={!p.chapterId}
            className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-2.5 py-1.5 transition-all hover:border-slate-400 hover:shadow-sm disabled:opacity-40 disabled:cursor-not-allowed">
            <panel.icon size={13} className="text-slate-500 shrink-0" />
